...
import {ethers, Wallet, Contract, ContractFactory, HDNodeWallet, BaseWallet} from '@axiomesh/axiom'
...
describe('test selfdestruct', () => {
    ...
    beforeAll(async () => {
        ...
        abi = fs.readFileSync(`${ST_CONTRACT_DIR}Selfdestruct/TestSelfDestruct.abi`, 'utf8')
        bytecode = fs.readFileSync(`${ST_CONTRACT_DIR}Selfdestruct/TestSelfDestruct.bin`, 'utf8')
    ...
    test('contract should receive ETH and self-destruct', async () => {
        ...
        const destroyReceipt = await contractOfSuicide.destroyContract(rWallet.address)
...
describe('test selfdestruct on creation', () => {
    ...
    test('test selfdestruct on creation and code will be 0x', async () => {
        provider = newProvider()
        wallet = new ethers.Wallet(ST_ADMIN_1.privateKey, provider)
        abi = fs.readFileSync(`${ST_CONTRACT_DIR}Selfdestruct/TestSelfDestructOnCreation.abi`, 'utf8')
        bytecode = fs.readFileSync(`${ST_CONTRACT_DIR}Selfdestruct/TestSelfDestructOnCreation.bin`, 'utf8')
